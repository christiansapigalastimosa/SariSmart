...
const STORAGE_KEY = 'SmartCanteen:orders';
let orders: Order[] = [];
let nextId = 1;
...
export async function setOrderItemPrepared(orderId: number, productId: number, prepared: boolean): Promise<void> {
  const order = orders.find((entry) => entry.id === orderId);
  if (!order) return;
...
export function isOrderReady(order: Order): boolean {
  return order.items.length > 0 && order.items.every((product) => product.prepared === true);
}
...
export async function saveReport(report: any): Promise<void> {
  const key = 'SmartCanteen:reports';
  try {
...
export async function moveReportToTrash(index: number): Promise<any[]> {
  const key = 'SmartCanteen:reports';
  const trashKey = 'SmartCanteen:reports_trash';
  try {
    const raw = await AsyncStorage.getItem(key);
