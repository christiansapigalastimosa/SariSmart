...
export default function Staff() {
  ...
  async function handleProcess(orderId: number) {
    setConfirmState({
      title: 'Process order',
      message: `Send order #${orderId} to cashier for payment?`,
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <View style={[bento.card, styles.card]}>
            ...
            <AppButton title="View details" onPress={() => router.push({ pathname: "/staff/[id]", params: { id: item.id.toString() } })} variant="secondary" style={[styles.actionButton, styles.smallButton]} />
            <AppButton
              title="Process & send to cashier"
              onPress={() => handleProcess(item.id)}
              disabled={!item.items.every((product) => Boolean(product.prepared))}
